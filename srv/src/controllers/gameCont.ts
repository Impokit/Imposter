...
export const handleSocketConnection = async (socket: WebSocket) => {
    ...
    socket.on('message', (data) => {
        ...
        if (msg.type === 'joinGame') {
            ...
            if (!game) return socket.send(JSON.stringify({ type: 'error', message: 'Game not found' }));
            ...
            if (!player) {
                ...
                if (game.players.includes(player)) return socket.send(JSON.stringify({ type: 'error', message: 'Player already joined' }));
    ...
    socket.on('close', () => {
        ...
        broadcast(gameId, { type: 'playerLeft', player: playerId, players: game.players });
